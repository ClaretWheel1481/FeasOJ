...
export const loginRequest = async (username, password) => {
    return await axios.get(`${apiUrl}v2/login`, {
        params: {
            username: username,
            password: password
        }
...
export const getCaptchaCode = async (email) => {
    return await axios.get(`${apiUrl}v2/captcha`, {
        params: {
            email: email
        }
    });
...
export const getUserInfo = async (username) => {
    return await axios.get(`${apiUrl}v2/users/${username}`);
}

// 获取题目列表
export const getAllProblems = async () => {
    return await axios.get(`${apiUrl}v2/problems`);
}

// 获取题目详细信息
export const getPbDetails = async (Pid) => {
    return await axios.get(`${apiUrl}v2/problems/${Pid}`);
}

// 获取讨论列表
export const getAllDis = async (page, itemsPerPage) => {
    return await axios.get(`${apiUrl}v2/discussions`, {
        params: {
            page: page,
...
export const getDisDetails = async (Did) => {
    return await axios.get(`${apiUrl}v2/discussions/${Did}`)
}

// 获取指定用户提交记录
export const getUserSubmitRecords = async (username) => {
    return await axios.get(`${apiUrl}v2/users/${username}/submitrecords`)
}

// 获取30天内的提交记录
export const getSubmitRecords = async () => {
    return await axios.get(`${apiUrl}v2/submitrecords`)
}
...
export const uploadCode = async (file, pid, username, token) => {
    ...
    return await axios.post(`${apiUrl}v1/users/code`, formData, {
        params: {
            'problem': pid
        },
        headers: {
...
export const updateSynopsis = async (username, token, synopsis) => {
    ...
    return await axios.put(`${apiUrl}v1/users/synopsis`, formData, {
        headers: {
            username: encodeURIComponent(username),
            Authorization: token,
        }
...
export const uploadAvatar = async (file, username, token) => {
    ...
    return await axios.put(`${apiUrl}v1/users/avatar`, formData, {
        headers: {
            'Content-Type': 'multipart/form-data',
            username: encodeURIComponent(username),
            Authorization: token
...
export const updatePassword = async (email, vcode, newPassword) => {
    return await axios.put(`${apiUrl}v2/users/password`, {
        email: email,
        vcode: vcode,
